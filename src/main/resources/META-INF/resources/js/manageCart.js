...
function storeProduct(productName, productPrice, imageId, quantity = 1) {
    ...
    let product = {name: productName, price: productPrice, imageId: imageId, quantity : 1};
    if (cart === null) {
        ...
        for (let product of cart) {
            if (product.name === productName) {
                ...
                if (product.quantity > 0) {
                    ...
                    const quantityLayout = articleLayout.getElementsByClassName(PRODUCT_QUANTITY_CLASSNAME)[0];
                    quantityLayout.value = Number(quantityLayout.value) + productQuantity;

                    // Update view to the new total price
                    const price = parseInt(articleLayout.getElementsByClassName(PRODUCT_PRICE_CLASSNAME)[0].innerHTML);
                    const totalLayout = articleLayout.getElementsByClassName(PRODUCT_TOTAL_CLASSNAME)[0];
                    totalLayout.innerHTML = (Number(quantityLayout.value) * price).toString()
                } else {
                    // Filter the cart with product.quantity <= 0
                    cart = cart.filter((product) => { return product.quantity > 0;});
    ...
    if (cart !== null) {
        for (let product of cart) {
            ...
            document.getElementById("cart_list").innerHTML +=
                ...
                "  <img src=\"/image/" + product.imageId + "\" alt=\"Oups missing image\">\n" +
                " </td>\n" +
                ...
                "   <a class=\"cart_quantity_up\" onclick=\"changeProductQuantity(\'" + name + "\', 1)\"> + </a>\n" +
                "   <input class=\"cart_quantity_input\" type=\"text\" name=\"quantity\" value=\"" + quantity + "\" autocomplete=\"off\" size=\"2\">\n" +
                "   <a class=\"cart_quantity_down\" onclick=\"changeProductQuantity(\'" + name + "\', -1)\"> - </a>\n" +
                "  </div>\n" +
                " </td>\n" +
                " <td class=\"cart_total\">\n" +
                ...
                "  <a class=\"cart_quantity_delete\" onclick=\"removeProduct(\'" + name + "\')\"><i class=\"fa fa-times\"></i></a>\n" +
                " </td>\n" +
                "</tr>";
        }
    }
...
function removeProduct(productName) {
    ...
    if (cart !== null) {
        // Filter the cart
        cart = cart.filter((product) => { return product.name !== productName;});
